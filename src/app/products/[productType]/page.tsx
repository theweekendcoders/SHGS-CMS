...
const page = async ({ params }: any) => {
  ...
  const Details = JSON.parse(params.orderSummary);
            ...
            <div className="my-4 block w-full overflow-x-auto  overflow-y-auto shadow-lg rounded-sm">
              <table className="items-center w-full bg-transparent border-collapse">
                <thead className="border-b-2 border-black w-full p-10">
                  <tr>
                    <th className="px-6 align-middle py-3 text-md capitalize whitespace-nowrap font-semibold text-left">
                      S.No
                    </th>
                    <th className="px-6 align-middle py-3 text-md capitalize whitespace-nowrap font-semibold text-left">
                      Product
                ...
                <tbody className="">
                  {products.map((item: any, index: any) => (
                    <tr key={index} className="border-b-2">
                      <td className="px-6 align-middle font-medium text-md p-4 text-left items-center">
                        {index + 1}
                      </td>
